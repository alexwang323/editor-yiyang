...
void main() {
	vec3 FragPos = texture(g_position, i_texcoord).rgb;
	vec3 N = texture(g_normal, i_texcoord).rgb;
	vec3 Kd = texture(g_albedo, i_texcoord).rgb;
	vec3 Ks = texture(g_specular, i_texcoord).rgb;
    float alpha = texture(g_specular, i_texcoord).a;
    alpha = alpha * alpha;
