...
void main() {
    ...
    for (int i = 0; i < u_samples; ++i) {
	    ...
	    vec3 FragPos = texelFetch(g_position_ms, uv_pos, i).rgb;
	    vec3 N = texelFetch(g_normal_ms, uv_nrm, i).rgb;
	    vec3 Kd = texelFetch(g_albedo_ms, uv_col, i).rgb;
        //float alpha = texelFetch(g_albedo_ms, uv_col, i).a;
	    vec3 Ks = texelFetch(g_specular_ms, uv_spc, i).rgb;
        float alpha = texelFetch(g_specular_ms, uv_spc, i).a;
        alpha = alpha * alpha;
        ...
        if (FragPos.z == 1.0)
            FragColor += vec4(FragColor.xyz, 0.0);
        else
